...
<div class="container">
  <dvi class="col-md-offset-2 col-md-8">
     ...
     {!! Form::open(['class'=>'form-horizontal', 'method'=>'patch', 'files' => true, 'action'=>['UsersController@update',$human->id ]]) !!}

     <div class="panj jel-body p25 bg-light">
     <div style="background-color: " class="text-center section-divider mt20 mb50">
     <span><h2>EDIT USER ACCOUNT</h2></span>
     </div>
     ...
     <div class="col-md-4">
       ...
       <div class="fileinput text-center fileinput-new" data-provides="fileinput">
             <div class="fileinput-new thumbnail img-circle">
                 <img src="{{url($human->profile_image)}}" alt="...">
             </div>
             <div>
     ...
     <div class="col-md-6">
       ...
       <div class="form-group is-empty form-group{{ $errors->has('title') ? ' has-error' : '' }}">
            ...
            <select type="text" id="title" class="form-control"  placeholder="" name="title" value="" required>
              <option value="" selected>Select Title</option>
              <option value="Mr" @if($human->title == 'Mr') selected @endif>Mr</option>
              <option value="Mrs" @if($human->title == 'Mrs') selected @endif>Mrs</option>
              <option value="Miss" @if($human->title == 'Miss') selected @endif>Miss</option>
              <option value="Engr" @if($human->title == 'Engr') selected @endif>Engr</option>
            </select>
    ...
    <!--  <div class="form-group">
     ...
     <div class="form-group is-empty form-group{{ $errors->has('phone_number') ? ' has-error' : '' }}">
          ...
          <input type="number" name="phone_number" class="form-control" value="{{ $human->phone_number }}" required>
     ...
     <div class="form-group is-empty form-group{{ $errors->has('gender') ? ' has-error' : '' }}">
          ...
          <select type="text" id="gender" class="form-control"  placeholder="" name="gender" value="{{ old('gender') }}" required>
                ...
                <option value="Male" @if($human->gender == 'Male') selected @endif>Male</option>
               <option value="Female" @if($human->gender == 'Female') selected @endif>Female</option>
          </select>
     ...
     <button type="submit" class="btn btn-round btn-info pull-right">Update Account</button>
     </div>
     </div></div>
